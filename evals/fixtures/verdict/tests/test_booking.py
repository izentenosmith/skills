...
# --- cutoff window tests (added this cycle) ---

def in_minutes(m):
    return datetime.now() + timedelta(minutes=m)


def test_booking_inside_cutoff_window_is_rejected(service):
    with pytest.raises(Rejected) as e:
        service.submit("acct-1", "venue-1", in_minutes(30), 2)
    assert "CUTOFF_WINDOW" in e.value.codes


def test_booking_outside_cutoff_window_is_accepted(service):
    booking = service.submit("acct-1", "venue-1", in_minutes(300), 2)
    assert booking.party_size == 2


def test_cutoff_window_reads_from_account_config():
    store = FakeStore()
    service = BookingService(store, FakeConfig({"cutoff_minutes": 240}))
    with pytest.raises(Rejected) as e:
        service.submit("acct-1", "venue-1", in_minutes(30), 2)
    assert "CUTOFF_WINDOW" in e.value.codes
