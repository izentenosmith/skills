...
description: Staff-architect design review that resolves a plan one question at a time, inferring answers from the codebase where possible. Use to map design trade-offs and establish an execution plan before writing a plan or code.
---
...
- [ ] **Data model.** New/changed entities, relationships, ownership, and invariants. Migration of existing data.
...
If the effort is large, also resolve **decomposition**: how it splits into independently shippable increments (each of which can become its own brief → TDD → refactor-review → devils-advocate → tyr-verdict pass).
...
> The design is resolved across all dimensions. Run **[agent-brief](../agent-brief/SKILL.md)** next to turn these decisions into the pure-text plan TDD builds from.
